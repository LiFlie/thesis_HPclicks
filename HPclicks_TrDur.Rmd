---
title: "HPclicks average Train Duration"
author: "Lina Fliess"
date: "`r Sys.Date()`"
...
```{r summarys DE}
summary(Ref_DE_Aschau$TrDur_us)
summary(Ref_DE_BoEck$TrDur_us)
summary(Ref_DE_Langholz$TrDur_us)
```

###Visualization/Plots:

```{r histogram DE, echo=FALSE}
ggplot(Ref_DE, aes(x = TrDur_us, fill = group)) +
  geom_histogram(bins = 50, alpha = 0.5, position = "identity") +
  labs(title = "Average Train Duration Distribution", x = "Average Train Duration", y = "Count") +
...
ggplot(Ref_DE, aes(x = group, y = TrDur_us, fill = group)) +
  geom_boxplot() +
...
ggplot(Ref_DE, aes(x = group, y = TrDur_us, color = group)) +
...
kruskal.test(data = Ref_DE, TrDur_us ~ group)
```
...
```{r summarys DK}
summary(Ref_DK_6621$TrDur_us)
summary(Ref_DK_6623$TrDur_us)
summary(Ref_DK_6625$TrDur_us)
```

###Visualization/Plots:

```{r histogram DK, echo=FALSE}
ggplot(Ref_DK, aes(x = TrDur_us, fill = group)) +
  geom_histogram(bins = 50, alpha = 0.5, position = "identity") +
  labs(title = "Average Train Duration Distribution", x = "Average Train Duration", y = "Count") +
  scale_fill_manual(name = "Dataset", 
                    values = c('6621' = 'gold', 
...
ggplot(Ref_DK, aes(x = group, y = TrDur_us, fill = group)) +
  geom_boxplot() +
  labs(title = "Boxplot of Average Train Duration", x = "Group", y = "Average Train Duration") +
  theme_minimal()
```
...
ggplot(Ref_DK, aes(x = group, y = TrDur_us, color = group)) +
  geom_jitter(width = 0.2, alpha = 0.5) +
  labs(title = "Scatter Plot of Average Train Duration", x = "Group", y = "Average Train Duration") +
  theme_minimal()
```
...
kruskal.test(data = Ref_DK, TrDur_us ~ group)
```
...
kruskal_result <- kruskal.test(data = Ref_both, TrDur_us ~ group)
```
